...
if ( !isset($_GET['TypeID']) OR !isset($_GET['TransNo']) ) {
	...
	if ( DB_num_rows($TypeResult) == 0 ){
		...
		echo '<tr>
				<th colspan="7"><h2><b>' . _('Trans Type').' '._($TransName) . ' '._('ChineseGL_di') . $_GET['TransNo'] ._('ChineseGL_Number'). '</b></h2></th>
			</tr>
			<tr>
				<th>' . _('Date') . '</th>
				<th>' . _('Description')  . '</th>
		...
		echo '<tr style="background-color:#FFFFFF">
				<td class="number" colspan="3"><b>' . _('ChineseGL_Total') . '</b></td>
...
list($userid,$realname,$stepdate)=getTransBy($_GET['TypeID'],$_GET['TransNo']);//∂¡»°
			echo '<tr style="background-color:#80B7FF">
				<td class="number" colspan="6"><b>' . _('ChineseGL_Sign:') .$realname. '</b></td>
			</tr>';
		echo '</table>';
	}
