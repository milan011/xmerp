...
if(isset($_POST['PDFWorkOrderReceived'])){
	include('PDFWorkOrderReceived.php');
	exit();
}
...
if (isset($_POST['Process'])){ //user hit the process the work order receipts entered.
	...
	if ($QuantityReceived + $WORow['qtyrecd'] < 0){
		prnMsg(_('Issue_Back_With_Nagetive_Num_But_No_More_Than_Issued_cqz'),'error');
		$InputError=true;
	}
...
//					$PeriodNo = GetPeriod(Date($_SESSION['DefaultDateFormat']), $db);
		...
		while ($AutoIssueCompRow = DB_fetch_array($AutoIssueCompsResult)){
			...
			*/
			...
			$isshow = getIsshow($StockID,$AutoIssueCompRow['stockid'],$db);
			...
			//if GL linked then do the GL entries to DR wip and CR stock
				...
				$SQL = "INSERT INTO gltrans (type,
							...
							VALUES (28,
								...
								'" . $PeriodNo . "',
								'" . $StockGLCode['wipact'] . "',
								'" . DB_escape_string($_POST['WO'] . ' - ' . $WORow['description'].'['.$_POST['StockID'] . '] ' . _('Component') . ': ' . $AutoIssueCompRow['description'].'['.$AutoIssueCompRow['stockid'] . '] - ' . $QuantityReceived . ' x ' . $AutoIssueCompRow['qtypu'] . ' @ ' . locale_number_format($AutoIssueCompRow['cost'],$_SESSION['CompanyRecord']['decimalplaces'])) . "',
				...
				$SQL = "INSERT INTO gltrans (type,
							...
							VALUES (28,
								...
								'" . DB_escape_string($_POST['WO'] . ' - ' . $WORow['description'].'['.$_POST['StockID'] . '] -> ' . $AutoIssueCompRow['description'].'['.$AutoIssueCompRow['stockid'] . '] - ' . $QuantityReceived . ' x ' . $AutoIssueCompRow['qtypu'] . ' @ ' . locale_number_format($AutoIssueCompRow['cost'],$_SESSION['CompanyRecord']['decimalplaces'])) . "',
								'" . -($AutoIssueCompRow['qtypu'] * $QuantityReceived * $AutoIssueCompRow['cost']) . "')";
		  ...
		  it is retrieved from the stock category record of the item by a function in SQL_CommonFunctions.inc*/
			...
			$SQL = "INSERT INTO gltrans (type,
							...
							VALUES (26,
								...
								'" . $PeriodNo . "',
								'" . $StockGLCode['stockact'] . "',
								'" . DB_escape_string($_POST['WO'] . " - " . $WORow['description'] . "[" . $_POST['StockID'] . '] x ' . $QuantityReceived . " @ " . locale_number_format($WORow['stdcost'],$_SESSION['CompanyRecord']['decimalplaces'])) . "',
	...
	if ($WORow['serialised']==1){ //serialised
		...
		echo '<br />
			<div class="centre">
				...
				<input type="submit" name="PDFWorkOrderReceived" value="' . _('Print Work Receipt Note_cqz') . '" />
			</div>';
	} else { //controlled but not serialised - just lot/batch control
		echo '<tr>
