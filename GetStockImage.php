...
/* $Id: GetStockImage.php 6312 2013-08-30 21:08:37Z daintree $*/

//include ('includes/session.inc');
include('includes/CommonsCQZ.inc');
if(isset($_GET['AssetID'])){
	if(isset($_SESSION['ASSET_CQZ'])){
		$sql = "SELECT archfile
			FROM fixedassets
			WHERE assetid ='" . $_SESSION['ASSET_CQZ'] . "'";

		$result = DB_query2($sql, $db);
		$AssetRow = DB_fetch_array($result);
		$filename = $AssetRow['archfile'];
		if(file_exists($filename)){
			if( '.pdf'==getWin32FileSuffix($filename) ){
	...
	$title = _('Asset Archive Downliad Script Problem');
	include('includes/header.inc');
	prnMsg(_('未指定资产'),'error');
