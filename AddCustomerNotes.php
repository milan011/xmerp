...
/* $Id: AddCustomerNotes.php 6310 2013-08-29 10:42:50Z daintree $*/
...
if (!isset($Id)) {
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		printf('<td>%s</td>
				...
				<td><a href="%sId=%s&amp;DebtorNo=%s&amp;delete=1" onclick="return confirm(\'' . _('Are you sure you wish to delete this customer note?') . '\');">' .  _('Delete'). '</a></td></tr>',
...
if (!isset($_GET['delete'])) {
	...
	echo '<td><input type="date" required="required" name="NoteDate" class="date" alt="' .$_SESSION['DefaultDateFormat']. '" id="datepicker" value="'.$_POST['NoteDate'].'" size="10" maxlength="10" /></td>
			</tr>';
	echo '<tr>
	...
	echo '<td><textarea name="Note" autofocus required="required" rows="3" cols="32">' .$_POST['Note'] . '</textarea></td>
			</tr>';
	echo '<tr>
			<td>' .  _('WWW') . '</td>';
	if (!isset($_POST['Href'])) {
	...
	echo '<td><input type="url" name="Href" value="'.$Href.'" size="35" maxlength="100" /></td>
			</tr>';
	echo '<tr>
			<td>' .  _('Priority'). '</td>';
	...
	echo '<td><input type="number" required="required" min="1" name="Priority" class="number" value="' . $_POST['Priority']. '" size="1" maxlength="3" /></td>
			</tr>';
	echo '<tr>
			<td colspan="2">
			<div class="centre">
