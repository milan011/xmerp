...
echo '<table cellpadding="2" class="selection">
	...
	$RequirmentsResult = DB_query("SELECT worequirements.stockid,
										...
										stockmaster.mbflag as subbom
									FROM worequirements INNER JOIN stockmaster
									ON worequirements.stockid=stockmaster.stockid
									INNER JOIN bom ON bom.parent=worequirements.parentstockid AND bom.component=worequirements.stockid
									WHERE bom.isshow=1 AND worequirements.wo='" . $SelectedWO . "' AND worequirements.parentstockid='".$StockID."'",
									$db);
	...
	while ($RequirementsRow = DB_fetch_array($RequirmentsResult)){
		...
		echo '<td align="right">' . locale_number_format($WORow['qtyreqd']*$RequirementsRow['qtypu'],$RequirementsRow['decimalplaces']) . '</td>
			<td align="right">' . locale_number_format($IssuedAlreadyRow[0],$RequirementsRow['decimalplaces']) . '</td>
			<td>';
		echo  ($RequirementsRow['subbom']<>'M')?'-':
		'<a href="' . $RootPath . '/WorkOrderEntry.php?NewItem=' . $RequirementsRow['stockid'] . '&amp;ReqQty=' . round($WORow['qtyreqd']*$RequirementsRow['qtypu'],$RequirementsRow['decimalplaces']) . '&amp;ReqDate=' . ConvertSQLDate($WORow['requiredby']) . '">' . _('Convert') . '</a>' ;
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		echo ($RequirementsRow['subbom']<>'M')?'-':
		'<td><a href="' . $RootPath . '/WorkOrderEntry.php?WO='.$SelectedWO.'&amp;NewItem=' . $RequirementsRow['stockid'] . '&amp;ReqQty=' . round($WORow['qtyreqd']*$RequirementsRow['qtypu'],$RequirementsRow['decimalplaces']) . '&amp;ReqDate=' . ConvertSQLDate($WORow['requiredby']) . '">' . _('Add') . '</a></td>';
		echo '</td></tr>';
	}
