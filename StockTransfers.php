...
/* $Id: StockTransfers.php 6310 2013-08-29 10:42:50Z daintree $*/
...
if ($NewTransfer){
	...
	if (DB_num_rows($result) == 0){
		...
		$_SESSION['Transfer']->TransferItem[0] = new LineItem (	trim(mb_strtoupper($_POST['StockID'])),
															$myrow['description'],
						 									filter_number_format($_POST['Quantity'],$myrow['decimalplaces']),
															$myrow['units'],
															$myrow['controlled'],
															$myrow['serialised'],
															$myrow['perishable'],
...
if (isset($_POST['Quantity'])
	...
	$_SESSION['Transfer']->TransferItem[0]->Quantity = filter_number_format($_POST['Quantity'],$_SESSION['Transfer']->TransferItem[0]->DecimalPlaces);
...
if ( isset($_POST['EnterTransfer']) ){
	...
	$result = DB_query("SELECT MBFlag FROM stockmaster WHERE stockid='" . $_SESSION['Transfer']->TransferItem[0]->StockID ."'",$db);
	...
	if (DB_num_rows($result)==0) {
		...
		prnMsg( _('A_Stock_cannot_transfer_cqz'),'error');
		$InputError = true;
	} elseif ($myrow[0]=='D'){
		prnMsg( _('D_Stock_cannot_transfer_cqz'),'error');
		$InputError = true;
	} elseif ($myrow[0]=='K'){
		prnMsg( _('K_Stock_cannot_transfer_cqz'),'error');
		$InputError = true;
