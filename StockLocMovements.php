<?php
/* $Id: StockLocMovements.php 6310 2013-08-29 10:42:50Z daintree $*/
...
echo ' ' . _('Show Movements before') .': <input type="text" name="AfterDate" size="12" maxlength="12" value="' . $_POST['AfterDate'] . '" />' ;
echo ' ' . _('But after') . ': <input type="text" name="BeforeDate" size="12" maxlength="12" value="' . $_POST['BeforeDate'] . '" />';
echo '</td>
     </tr>
     </table>
     <br />';
...
$sql = "SELECT stockmoves.stockid,
				...
				debtorsmaster.name as debtorname
        	FROM stockmoves
        	INNER JOIN systypes ON stockmoves.type=systypes.typeid
        	INNER JOIN stockmaster ON stockmoves.stockid=stockmaster.stockid
			LEFT JOIN debtorsmaster ON debtorsmaster.debtorno=stockmoves.debtorno
...
while ($myrow=DB_fetch_array($MovtsResult)) {
	...
	$DisplayTranDate = ConvertSQLDate($myrow['trandate']);
		...
		printf('<td><a target="_blank" href="' . $RootPath . '/StockStatus.php?StockID=%s">%s</a></td>
				...
				mb_strtoupper($myrow['description']),
				$myrow['typename'],
				$myrow['transno'],
				$DisplayTranDate,
				$myrow['debtorname'],
