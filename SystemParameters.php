...
/* $Id: SystemParameters.php 6428 2013-11-22 22:36:42Z rchacon $*/
...
//new number must be shown
		...
		if ($_SESSION['NoPriceNoSales'] != $_POST['X_NoPriceNoSales'] ) {
			$sql[] = "UPDATE config SET confvalue = '". ($_POST['X_NoPriceNoSales'])."' WHERE confname = 'NoPriceNoSales'";
		}//insert into config (confvalue,confname)values(1,'NoPriceNoSales');
...
// No Price No Sales
echo '<tr style="outline: 1px solid"><td>' . _('No Price No Sales') . ':</td>
	<td><select name="X_NoPriceNoSales">
	<option '.($_SESSION['NoPriceNoSales']==0?'selected="selected" ':'').'value="0">'._('Sales Any Way').'</option>
	<option '.($_SESSION['NoPriceNoSales']==1?'selected="selected" ':'').'value="1">'._('No Price No Sales').'</option>
	</select></td>
	<td>' . _('Sales forbidden when no prices set for the stock and the customer.') . '</td></tr>';
