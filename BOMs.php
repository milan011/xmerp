...
/* $Id: BOMs.php 6310 2013-08-29 10:42:50Z daintree $*/
...
if (isset($Select)) { //Parent Stock Item selected so display BOM or edit Component
    ...
    /* We do want to show the new component entry form in any case - it is a lot of work to get back to it otherwise if we need to add */
		...
		echo '<tr><td>' . _('Show this Component to Work Orders when print') . ':</td>
				<td>
				<select name="IsShow">';
		if (!isset($_POST['IsShow'])){//ALTER TABLE `bom` ADD COLUMN `isshow` TINYINT NOT NULL DEFAULT '1' AFTER autoissue`;
			$_POST['IsShow'] = 1;
		}
		if ($_POST['IsShow']==0) {
			echo '<option selected="selected" value="0">' . _('No') . '</option>';
