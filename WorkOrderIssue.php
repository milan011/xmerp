...
include('includes/WorkOrderFunctions.inc');
...
/************************ BEGIN SQL TRANSACTIONS ************************/
		...
		$isshow = getIsshow($_POST['StockID'],$_POST['IssueItem'],$db);
		  ...
		  it is retrieved from the stock category record of the item by a function in SQL_CommonFunctions.inc*/
			...
			$SQL = "INSERT INTO gltrans (type,
					...
					VALUES (28,
						...
						'" . $PeriodNo . "',
						'" . $WORow['wipact'] . "',
						'" . $_POST['WO'] . " " . $IssueItemRow['description'].'['.$_POST['IssueItem'] . '] x ' . $QuantityIssued . " @ " . locale_number_format($IssueItemRow['cost'], $_SESSION['CompanyRecord']['decimalplaces']) . "',
...
echo '<table class="selection">
		<tr>
			...
			<td>' . $_POST['WO'] .'</td>
			<td><input type="submit" name="PDFPrint" value="'._('Print To PDF').'"></td>
...
if (!isset($_POST['IssueItem'])){ //no item selected to issue yet
	...
	$SQL_tmp="SELECT worequirements.stockid,stockmaster.units,
										stockmaster.description,
										stockmaster.decimalplaces,
										worequirements.autoissue,
										bom.isshow,
	...
	if(isset($_POST['PDFPrint'])){
		$SQL_tmp.=' AND isshow=1';
	}elseif(isset($_POST['PDFPrint2'])){
		$SQL_tmp.=' AND isshow=1';
	}else{
		$SQL_tmp.=' AND isshow=1';
	}
	$SQL_tmp.=' GROUP BY worequirements.stockid,
											stockmaster.description,
											stockmaster.decimalplaces,
											autoissue';
	$RequirmentsResult = DB_query($SQL_tmp,
									$db);
	if(isset($_POST['PDFPrint'])||isset($_POST['PDFPrint2'])){
		ob_end_clean();
		include('WorkOrderIssue_pdf.php');
		exit();
	}
	$k=0; //row colour counter
	while ($RequirementsRow = DB_fetch_array($RequirmentsResult)){
		...
		if ($RequirementsRow['autoissue']==0){
			echo '<td><input type="submit" name="IssueItem" value="' .$RequirementsRow['stockid'] . '" /></td>
					<td>' . $RequirementsRow['stockid'] . ' - ' . $RequirementsRow['description'] . '</td>';
		} else {
			echo '<td class="notavailable">' . _('Auto Issue') . '</td>
		...
		echo '<td class="number">' . locale_number_format($RequirementsRow['quantityrequired'],$RequirementsRow['decimalplaces']) . '</td>
			...
			<td class="number">' .($RequirementsRow['isshow']==0?_('No'):_('Yes'))  . '</td>
		</tr>';
	}
	...
	if ($IssueItemRow['controlled']==1){ //controlled
		...
		echo '<tr><td>' . _('Quantity Issued') . ':</td>
			  <td><input class="number" type="text" size="10" maxlength="10" title="' . _('Enter the quantity of this item to issue to the work order') . '" name="Qty" required="required" value="0"/></td></tr>';
		echo '<tr>
