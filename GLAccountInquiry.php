...
if (isset($_POST['Show'])){
	...
	echo '<tr style="background-color:#FDFEEF"><td colspan="4"><b>';
	if ($PandLAccount==True){
	...
	if ($RunningTotal >0){
		echo '<td class="number"><b>' . locale_number_format(($RunningTotal),$_SESSION['CompanyRecord']['decimalplaces']) . '</b></td><td></td><td colspan="3"></td></tr>';
	}else {
		echo '<td></td><td class="number"><b>' . locale_number_format((-$RunningTotal),$_SESSION['CompanyRecord']['decimalplaces']) . '</b></td><td colspan="3"></td></tr>';
	}
	echo '</table>';
