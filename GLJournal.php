...
/* $Id: GLJournal.php 4841 2012-01-27 10:12:31Z daintree $*/
...
echo '<table>
        <tr>';
//echo '<td colspan="5"><table class="selection">';
//echo '						<tr>';
echo '						<td>' . _('Date to Process Journal') . ':<input type="text" required="required" class="date" alt="' . $_SESSION['DefaultDateFormat'] . '" name="JournalProcessDate" maxlength="10" size="11" value="' . $_SESSION['JournalDetail']->JnlDate . '" /></td>
							<td>' . _('Type') . ':<select name="JournalType">';
...
echo '<td>' . _('GL Account Code') . ':<input class="number" type="text" autofocus="autofocus" name="GLManualCode" maxlength="12" size="12" onchange="inArray(this.value, GLCode.options,'.	"'".'The account code '."'".'+ this.value+ '."'".' doesnt exist'."'".')" value="'. $_POST['GLManualCode'] .'"  />';
...
$result=DB_query($sql, $db);
echo '' . _('Select GL Account') . ':<select name="GLCode" onchange="return assignComboToInput(this,'.'GLManualCode'.')">';
...
echo '</tr>
	<tr>
		<td>' . _('Debit') . ':<input type="text" class="number" name="Debit" onchange="eitherOr(this,Credit)" maxlength="12" size="10" value="' . locale_number_format($_POST['Debit'],$_SESSION['CompanyRecord']['decimalplaces']) . '" /></td>
		<td>' . _('GL Narrative') . ':<input type="text" name="GLNarrative" maxlength="100" size="50" value="' . $_POST['GLNarrative'] . '" /></td>
	</tr>
	<tr>
		<td>' . _('Credit') . ':<input type="text" class="number" name="Credit" onchange="eitherOr(this,Debit)" maxlength="12" size="10" value="' . locale_number_format($_POST['Credit'],$_SESSION['CompanyRecord']['decimalplaces']) . '" /></td>
		<td></td>
	</tr>';
/*	<tr>
		<td></td>
...
echo '<tr class="EvenTableRows"><td></td>
		...
		<td class="number"><b>' . locale_number_format($CreditTotal,$_SESSION['CompanyRecord']['decimalplaces']) . '</b></td><td></td>
	</tr>';
