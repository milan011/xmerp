...
if (!isset($_POST['StockLocation'])){
	...
	$_SESSION['UserStockLocation']=$_POST['StockLocation'];
}

//echo isset($_POST['Search']) ,',', isset($_POST['Prev']) ,',', isset($_POST['Next']);
...
if (isset($_POST['Search']) Or isset($_POST['Prev']) Or isset($_POST['Next'])){
	...
	//echo $sql,'<br/>',$Offset,',',$ListCount,',',$ListPageMax,',',$_SESSION['DisplayRecordsMax'],'<br/>';
	$SQL=$sql;
	//sjx
...
function lastWOEmpty($db){

	$result = DB_query("select max(wo) as maxwo from workorders",$db);
	$myrow=DB_fetch_array($result);
	DB_free_result($result);
	$maxwo=$myrow['maxwo'];
	if (empty($maxwo)) return true;
	$result = DB_query("select 1 from woitems where wo=($maxwo) limit 1",$db);
