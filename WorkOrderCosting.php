...
while ($WORow = DB_fetch_array($WOItemsResult)){
	if(isset($_POST['Close'])&&($WORow['qtyreqd']>$WORow['qtyrecd'])){
		$SomeClass=' style="background-color:#EEAABB"';
		$AllEnough = false;
	}else{
...
$RequirementsResult = DB_query("SELECT worequirements.stockid,
								...
								INNER JOIN bom 
									ON bom.parent=worequirements.parentstockid 
									AND bom.component=worequirements.stockid
								WHERE bom.isshow=1 AND worequirements.wo='" . $_POST['WO'] . "
	...
	if (DB_num_rows($IssuesResult)>0){
		while ($IssuesRow = DB_fetch_array($IssuesResult)){
			...
			echo '<td colspan="4"></td><td>' . ConvertSQLDate($IssuesRow['trandate']) . '</td>
				<td class="number"><a href="GLTransInquiry.php?TypeID=28&amp;TransNo='.$IssuesRow['transno'].'" target="wocstgl" title="'._('View GL Postings').'">' . locale_number_format(-$IssuesRow['qty'],$RequirementsRow['decimalplaces']) . '</a></td>
...
if (isset($_POST['Close'])&& (isset($_POST['MustClose'])&&$_POST['MustClose']=='on')) {
				...
				if ($ProportionOnHand < 1){
					...
					$SQL = "INSERT INTO gltrans (type,
									...
									VALUES (29,
										...
										'" . $_POST['WO'] . ' - ' . $WORow['description'].'['.$WORow['stockid'] . '] ' . _('share of variance') . "',
			...
			if ($_SESSION['CompanyRecord']['gllink_stock']==1 AND $TotalUsageVar!=0){
				...
				$SQL = "INSERT INTO gltrans (type,
						...
						VALUES (29,
							...
							'" . $PeriodNo . "',
							'" . $WORow['materialuseagevarac'] . "',
							'" . $_POST['WO'] . ' - ' . $WORow['description'].'['.$WORow['stockid'] . '] ' . _('share of usage variance') . "',
			...
			if ($_SESSION['CompanyRecord']['gllink_stock']==1 AND $TotalCostVar!=0){
				...
				$SQL = "INSERT INTO gltrans (type,
						...
						VALUES (29,
							...
							'" . $_POST['WO'] . ' - ' . $WORow['description'].'['.$WORow['stockid'] . '] ' . _('share of cost variance') . "',
...
if ($WorkOrderRow['closed']==0){
	echo '<tr>
			<td colspan="9">
				<div class="centre">';
	if (isset($_POST['Close'])&& (!isset($_POST['MustClose'])||$_POST['MustClose']!='on')) {
		echo '<input type="checkbox" name="MustClose">'._('Red_Meaning_Issue_Not_Enough_cqz').'<br/>';
		$confirmIt = '';
	}else{
		$confirmIt = ' onclick="return confirm(\'' . _('Closing the work order takes the variances to the general ledger (if integrated). The work order will no longer be able to have manufactured goods received entered against it or materials issued to it.') . '  ' . _('Are You Sure?') . '\');" ';
	}
	echo '<input type="submit" name="Close" value="' . _('Close This Work Order') . '" '.$confirmIt.'/>
				</div>
			</td>
		</tr>';
} else {
