...
/* $Id: PrintCustTransPortrait.php 4756 2011-11-27 02:36:47Z daintree $ */
...
If (isset($PrintPDF)
	...
	Class Cpdf1 extends Cpdf {
		...
		$this->Cell(0, 10, _(' ').$this->getAliasNumPage().('/').$this->getAliasNbPages().(''), 0, false, '', 0, '', 0, false, 'T', 'M');
		//$this->SetY(- 40);//y向位置
		//$this->SetX(- 50);//X向位置
		//$this->Cell(0, 10, _('共').$this->getAliasNbPages().('页'), 0, false, '', 0, '', 0, false, 'T', 'M');
		}
	}
	
	$pdf = new Cpdf1('L', 'pt', 'Z1');
	//$pdf = new Cpdf('L', 'pt', '1/3/A4');
	$pdf->addInfo('Author','cncERP ' );
	$pdf->addInfo('Creator','cnccERP http://www.cncerp.com');
	$pdf->addInfo('Subject',_('交易分录——收据或退货单——登录系统打印或者下载用户：').$_SESSION['UsersRealName']);
...
// gather the invoice data
	   ...
	   if (DB_error_no($db)!=0) {
		...
		if (DB_num_rows($result)>0){
		    ...
		    while ($myrow2=DB_fetch_array($result)){
				...
				if ($myrow2['decimalplaces']>2){
					$DisplayQty = locale_number_format($myrow2['quantity'],2);
				}else{
					$DisplayQty = locale_number_format($myrow2['quantity'],$myrow2['decimalplaces']);
				}
				$LeftOvers = $pdf->addTextWrap($Left_Margin+5,$YPos+3,71,$FontSize,$myrow2['stockid'],'center');//以下+3提升表内内容字符位置
				$LeftOvers = $pdf->addTextWrap($Left_Margin+90,$YPos+3,186,$FontSize,$myrow2['description']);
				$lines=0.37;//单据内容行距
				...
				$LeftOvers = $pdf->addTextWrap($Left_Margin+260,$YPos+3,76,$FontSize,$DisplayPrice,'right');
				$LeftOvers = $pdf->addTextWrap($Left_Margin+350,$YPos+3,36,$FontSize,$DisplayQty,'right');
				$LeftOvers = $pdf->addTextWrap($Left_Margin+390,$YPos+3,26,$FontSize,$myrow2['units'],'center');
				$LeftOvers = $pdf->addTextWrap($Left_Margin+420,$YPos+3,26,$FontSize,$DisplayDiscount,'right');
				$LeftOvers = $pdf->addTextWrap($Left_Margin+450,$YPos+3,72,$FontSize,$DisplayNet,'right');
				...
				if ($myrow2['controlled']==1){
					...
					if ($myrow2['serialised']==1){
						while ($ControlledMovtRow = DB_fetch_array($GetControlMovts)){
							$YPos -= (7*$lines);
							$LeftOvers = $pdf->addTextWrap($Left_Margin+82,$YPos+3,100,$FontSize,$ControlledMovtRow['serialno'],'left');
							if ($YPos-$line_height <= $Bottom_Margin){//批次号
						...
						while ($ControlledMovtRow = DB_fetch_array($GetControlMovts)){
							$YPos -= (7*$lines);
							$LeftOvers = $pdf->addTextWrap($Left_Margin+82,$YPos+3,100,$FontSize, _('批次号：').$ControlledMovtRow['serialno']. ' x ' . (-$ControlledMovtRow['moveqty']) ,'left');//批次号
				...
				$lines=explode('\r\n',htmlspecialchars_decode($myrow2['narrative']));//摘要
				...
				while (mb_strlen($lines[$i])>1){
					if ($YPos-$line_height <= $Bottom_Margin){
			   		...
			   		if (mb_strlen($lines[$i])>1){
						$lines[$i] = $pdf->addTextWrap($Left_Margin+85,$YPos+3,181,$FontSize,stripslashes($lines[$i]));
					}
					$YPos -= ($line_height);
				}
				}
		...
		$pdf->line($Left_Margin+78, $TopOfColHeadings+12,$Left_Margin+78,$Bottom_Margin+(1*$line_height));

		/*Print a column vertical line */
		$pdf->line($Left_Margin+268, $TopOfColHeadings+12,$Left_Margin+268,$Bottom_Margin+(1*$line_height));

		/*Print a column vertical line */
		$pdf->line($Left_Margin+348, $TopOfColHeadings+12,$Left_Margin+348,$Bottom_Margin+(1*$line_height));

		/*Print a column vertical line */
		$pdf->line($Left_Margin+388, $TopOfColHeadings+12,$Left_Margin+388,$Bottom_Margin+(1*$line_height));

		/*Print a column vertical line */
		$pdf->line($Left_Margin+418, $TopOfColHeadings+12,$Left_Margin+418,$Bottom_Margin+(1*$line_height));

		$pdf->line($Left_Margin+448, $TopOfColHeadings+12,$Left_Margin+448,$Bottom_Margin+(1*$line_height));

		/*Rule off at bottom of the vertical lines */
		$pdf->line($Left_Margin, $Bottom_Margin+(1*$line_height),$Page_Width-$Right_Margin,$Bottom_Margin+(1*$line_height));
		...
		if ($InvOrCredit=='Invoice') {
			 ...
			 $DisplayTotal2 = number_format($myrow['ovfreight']+$myrow['ovgst']+$myrow['ovamount'],2,".","");//大写金额去千分号，2012.2.4晚
			list($userid,$realname,$stepdate)=getTransBy(10,$FromTransNo);
			$Xitongqianmin=$realname;
		} else {
		     $DisplaySubTot = locale_number_format(-$myrow['ovamount'],$myrow['decimalplaces']);
		     $DisplayFreight = locale_number_format(-$myrow['ovfreight'],$myrow['decimalplaces']);
		     $DisplayTax = locale_number_format(-$myrow['ovgst'],$myrow['decimalplaces']);
		     $DisplayTotal = locale_number_format(-$myrow['ovfreight']-$myrow['ovgst']-$myrow['ovamount'],$myrow['decimalplaces']);
			 $DisplayTotal2 = number_format(-$myrow['ovfreight']-$myrow['ovgst']-$myrow['ovamount'],2,".","");//大写金额去千分号，2012.2.4晚
			list($userid,$realname,$stepdate)=getTransBy(11,$FromTransNo);
			$Xitongqianmin=$realname;
		}
	...
	if(strcmp($_SESSION['UsersRealName'],$Xitongqianmin)==0){
		
	$pdf->addTextWrap($Left_Margin+5,$YPos-69,680,$FontSize,_('提货人') . ': '.'                                '._('电话') . ': '.'                                 '._('会计') .': '.'                                '._('仓库') .': '.'                                '._('系统制单') . ': ' .$Xitongqianmin);
	}else{
	$pdf->addTextWrap($Left_Margin+5,$YPos-69,680,$FontSize,_('提货人') . ': '.'                                '._('电话') . ': '.'                                 '._('仓库') .': '.'                                '._('打印人') . ': '._($_SESSION['UsersRealName']).'                           '._('系统制单') . ': ' .$Xitongqianmin);
	}
	//	$FontSize =8;
...
//
	...
	//	$pdf->line($Page_Width-$Right_Margin-222, $YPos+$line_height,$Page_Width-$Right_Margin-222,$Bottom_Margin);
		...
		if ($InvOrCredit=='Invoice'){
			...
			$pdf->addText($Page_Width-$Right_Margin-520, ($YPos+2) - ($line_height*3),$FontSize, _('总计金额:'.$myrow['currcode']).'('.'大写)'.ChangeToRMB($num).'（含税：￥'.$DisplayTax.'元）','right');//总计金额
//			$FontSize=12;
...
//			$FontSize=10;
    		...
    		$pdf->addText($Page_Width-$Right_Margin-520, ($YPos+2) - ($line_height*3),$FontSize, _('注：无手工签字无效/冲销金额:'.$myrow['currcode']).'('.'大写)'.ChangeToRMB($num),'right');//总计金额
 		}
		$LeftOvers = $pdf->addTextWrap($Left_Margin+330,33,192, $FontSize, _('('.'小写)').'￥'.$DisplayTotal.'元' ,'right');//小写金额合计
	...
	/*if FromTransNo is not set then show a form to allow input of either a single invoice number or a range of invoices to be printed. Also get the last invoice number created to show the user where the current range is up to */
		...
		echo '<a href="PrintCustTransPortrait.php">返回</a>';
