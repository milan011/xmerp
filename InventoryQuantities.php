...
If (isset($_POST['PrintPDF'])) {
	...
	if ($_POST['Location'] != 'All') {
	    $WhereCategory .= " AND locstock.loccode='" . $_POST['Location'] . "'";
		$sql= "SELECT loccode, 
				locationname 
			FROM locations  
		...
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		$CatDescription .= ' '._('At Location').' '.$myrow[1];
	}
	...
	While ($myrow = DB_fetch_array($result,$db)){
	      if ($myrow['stockid'] != $holdpart) {
			...
			// and False to set to transparent
				...
				$pdf->addTextWrap(310,$YPos,60,$FontSize,$myrow['locationname'],'left',0);
...
echo '<div class="page_help_text">' . _('Use this report to display the quantity of Inventory items in different categories.') . '</div><br />';
	...
	while ($myrow1 = DB_fetch_array($result1)) {
		if ($myrow1['loccode']==$_POST['Location']){
